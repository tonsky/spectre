...
To set up sessions, create a :class:`SessionMiddleware` instance, specify its :attr:`~SessionMiddleware.sessionStore` attribute and add it to the :attr:`Settings.middlewares` list. For now, Spectre offers only one type of store: :class:`InmemorySessionStore`.
